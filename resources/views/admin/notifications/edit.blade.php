...
<div class="modern-form-container">
    <div class="form-header-section">
        <div class="header-content">
            ...
            <div>
                <h1 class="header-title">Edit Push Notification</h1>
                <div class="hint">Update notification details</div>
            </div>
        </div>
        <div>
    ...
    <div class="modern-form-card">
        <form action="{{ route('push-notifications.update', $notification) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="form-grid">
                <div class="form-group-modern">
                    ...
                    <select name="audience" class="modern-select" required>
                        <option value="all" {{ old('audience', $notification->audience)==='all' ? 'selected' : '' }}>All Users</option>
                        <option value="vendor" {{ old('audience', $notification->audience)==='vendor' ? 'selected' : '' }}>Vendors</option>
                        <option value="serviceprovider" {{ old('audience', $notification->audience)==='serviceprovider' ? 'selected' : '' }}>Service Providers</option>
                        <option value="user" {{ old('audience', $notification->audience)==='user' ? 'selected' : '' }}>Customers</option>
                    </select>
                ...
                <div class="form-group-modern">
                    ...
                    <input type="text" name="title" value="{{ old('title', $notification->title) }}" class="modern-input" placeholder="Enter offer title" required>
                ...
                <div class="form-group-modern full-width" style="grid-column:1 / -1;">
                    ...
                    <textarea name="description" class="modern-textarea" placeholder="Write description..." required>{{ old('description', $notification->description) }}</textarea>
                ...
                <div class="form-group-modern">
                    ...
                    @if($notification->image)
                        <div class="current-image mt-2">
                            <img src="{{ asset('Notification_images/'.$notification->image) }}" class="thumb" alt="img">
                            <span class="hint">Current image</span>
                        </div>
                    @endif
            ...
            <div class="form-actions">
                ...
                <button type="submit" class="modern-btn modern-btn-primary"><i class="fas fa-save"></i> Update Notification</button>
            </div>
        </form>
    </div>
</div>
