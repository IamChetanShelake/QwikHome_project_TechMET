        ...
        <form method="POST" action="{{ route('services.services.update', $service) }}">
            ...
            <div class="form-group">
                ...
                <select name="subcategory_id" id="subcategory_id" class="filter-select">
                    ...
                    <option value="{{ $sub->id }}" {{ $service->subcategory_id == $sub->id ? 'selected' : '' }}>{{ $sub->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $service->name) }}" class="filter-input" required>
                @error('name')
                    <div class="error">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="number" name="price" id="price" value="{{ old('price', $service->price) }}" step="0.01" min="0" class="filter-input" required>
                @error('price')
                    <div class="error">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" name="duration" id="duration" value="{{ old('duration', $service->duration) }}" class="filter-input" placeholder="e.g., 2 hours">
                @error('duration')
                    <div class="error">{{ $message }}</div>
                @enderror
            </div>
